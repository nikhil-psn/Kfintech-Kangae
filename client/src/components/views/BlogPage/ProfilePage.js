...
;
    ...
    axios.get("/api/ideas/getcategories").then((response) => {
      if (response.data.success) {
        ...
        for (i=0;i<response.data.categories.length;i++){
          ...
          Categories.push({value:response.data.categories[i],label:response.data.categories[i]})
        }
    ...
    axios.get("/api/ideas/getstatuses").then((response) => {
      if (response.data.success) {
        ...
        for (i=0;i<response.data.statuses.length;i++){
          ...
          Statuses.push({value:response.data.statuses[i],label:response.data.statuses[i]})
        }
  ...
  const searchTriggered = ()=>{
    ...
    axios.post("/api/ideas/searchIdeas",{"searchStr":document.getElementById("searchBar").value}).then((response) => {
      if (response.data.success) {
  ...
  return (
    ...
    <div style ={{minWidth:"100vw",minHeight:"100%"}} className={classes.appMain} style={{backgroundColor :"#3C44B1"}}>
    <Header  style={{ minHeight:"100vh"}}/>
    <Paper className={classes.pageContent}>
        <Form style={{ height :"100%"}} >
        <   Grid container spacing={3}  style={{paddingBottom : "25px"}}>
                <Grid xs={3} style={{alignContent : "center" , paddingTop:"10.5px",paddingLeft:"15px"}}>
                {/* <InputBase
              ...
              <Grid xs={3} sm={3} >
                ...
                <Select id="dropdownSort" className="sort__by"
              ...
              />
                ...
                <Grid  className="postscontainer" xs={8} sm={8}  style={{paddingLeft:"10px" }}>
